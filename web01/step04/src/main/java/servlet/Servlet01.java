/* 서블릿 만들기
 ...
 => Servlet 인터페이스
    init() - 서블릿 실행을 최초로 요청 받으면, 해당 클래스의 인스턴스를 생성한 후 호출한다.
                ...
                딱 한 번 호출된다.
             *.* => 서블릿은 오직 한 개만 생성된다.
    service() - 클라이언트가 요청할 때 마다 호출 한다
                    서블릿이 해야할 일을 이 메서드에 작성한다.
    destroy() - 웹 애플리케이션을 멈추거나 서블릿 컨터이너 서버를 종료할 때
    ...
    * init(), service(), destroy 삼인방 =>
       서블릿의 생성과 실행, 소멸과 관련된 메서드라고 해서 "Lifecycle method"라 부른다.
    getServletInfo() - 웹 애플리케이션 관리 화면에서 서블릿의 정보를 출력할 떄 호출된다.
                             서블릿의 정보를 간단한 문자열로 표현하여 리턴한다.
                             개발자가 직접 이 메소드를 호출할 일이 거의 없다.
