/* 클라이언트기 보낸 데이터 읽기2
=> 멀티바이트 문자 데이턱 깨지는 현상 해결
이유:
  - request.getParameter()는 클라이언트가 보낸 문자열은 ISO-8859-1 이라고 간주한다.
  - 그리고 이 데이터를 유니코드로 바꾼다. => 왜? 자바는 내부에서 유니코드를 사용하기 떄문이다.
