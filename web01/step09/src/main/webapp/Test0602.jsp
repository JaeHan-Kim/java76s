...
<%-- 2) <jsp:useBean> 사용 
	=> 네임스페이스 "jsp" JSP에 내장되었기 떄문에 따로 선언하지 않는다.
	=> id : 레퍼런드 변수 이름이면서 값을 꺼낼 때 키로 사용된다. 
	=> class : 값의 타입. fully qualified name 이여야 한다. import해도 소용없다.
						 제네릭을 선언할 수 없다.
...
<%-- 위의 JSP 액션 태그는 다음 자바 코드와 같다.
	...
	if (name == null) {
		naems = new ArrayList<String>();
		application.SetAttrubute("names", names);
	}
--%>
