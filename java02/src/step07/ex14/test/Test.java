 ...
 * - 보통 메서드는 외부에서 자유롭게 이용할 수 있도록 완전 공개한다.
 * - 클래스 멤버에아무것도 붙이지 않으면 (default 라 부른다.)기본 접근만 허용한다.
 *   기본 접근이란? 같은 패키지 소속 클래스만 사용할 수 있다.
 * - 기본 접근을 더하여 자식 클래스까지 사용을 하락하고 싶다면 "protected"
 *   보호모드로 공개한다.
 *   인스턴스 변수에 대해 주로 사용한다.
 * - 오직 내부에서만 사용할 때는 바깥에서 보지 못하도록 막아라! (private)
 *   인스턴스 변수에 대해 적용할 때가 많고,
 *   가끔 인스턴스 메서드에 대해서도 적용할 때가 있다.s
 */
