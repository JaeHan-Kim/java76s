...
-- 조회 결과로 몬든 컬럼의 값을 선택한다.
-- 컬럼의 순서는 테이블을 정의할 떄 선언한 순서를 따른다.
select * from board;
...
-- 특정 문자만 와일드카드(_) 적용
insert into board(title, content, cre_dt) values('dddddd@test.com', 'xxxxx', now());
select bno, title, content, views
from board
where title like '___@test.com' ; -- @ 앞에 세 글자만 허용.
