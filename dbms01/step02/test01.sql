...
alter table test08 add column tel varchar(20);
...
-- 4) 테이블 컬럼 제거
	alter table test08 drop column email;
	alter table test08 drop tel; -- column 생략 가능
...
-- age 컬럼의 값은 모두 null이기 떄문에 타입을 변경해도 문제가 없다. 그래서 ok!
	 alter table test08
...
-- 데이터가 들어 있는 칼럼을 삭제하려 한다면? 다른 테이블과 연결된 컬럼이 아니라면 삭제된다.
