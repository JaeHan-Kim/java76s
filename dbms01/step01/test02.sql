...
/* key - 데이터를 식별하기 위한 컬럼
		1) Key: 데이터를 식별하기 위해 여라 개의 컬럼을 묶은 것.
		2) Candidate Key : Key 들 중에서 최소의 컬럼으로 묶은 것. 최소키, 후보키라 부른다.
		3) Primary Key : 후보키 중에서 관리자가 사용하기 위해서 선택한 키.
 */
-- student 테이블의 예.
-- 1) Key
			name, email
...
-- Student를 저장할 테이블을 정의하라!
-- 일렬 번호를 primary key로 갖도록 한다.
-- 컬럼명: sno
create table test02 (
	tno   int primary key,
	name  varchar(10),
